...
function readAbiString(
  ...
  try {
    if (write(pointer, size) !== PVST_OK) fail(`failed to read ${label}`);
...
function probeParameter(
  ...
  const parameterId = call("pvst_class_param_id")(classIndex, parameterIndex) >>> 0;
  const flags = call("pvst_class_param_flags")(classIndex, parameterIndex) >>> 0;
  const stepCount = call("pvst_class_param_step_count")(classIndex, parameterIndex) >>> 0;
  ...
  const titleSize = call("pvst_class_param_title_size")(classIndex, parameterIndex) >>> 0;
  ...
  if (stepCount > 0) {
    for (let value = 0; value <= stepCount; value += 1) {
      ...
      const size = call("pvst_class_param_value_text_size")(classIndex, parameterIndex, normalized) >>> 0;
...
export async function probeWasm(wasm: Uint8Array): Promise<ProbedClass[]> {
  ...
  const count = requiredFunction(exports, "pvst_class_count")() >>> 0;
  const classes: ProbedClass[] = [];
  const classUids = new Set<string>();
  for (let classIndex = 0; classIndex < count; classIndex += 1) {
    const call = (name: string) => requiredFunction(exports, name);
    const uid = validatedUid(readAbiString(memory, malloc, free, call("pvst_class_uid_size")(classIndex) >>> 0,
    ...
    classUids.add(uid);
    const name = readAbiString(memory, malloc, free, call("pvst_class_name_size")(classIndex) >>> 0,
      (pointer, capacity) => call("pvst_class_name_write")(classIndex, pointer, capacity), `class ${uid} name`);
    const vendor = readAbiString(memory, malloc, free, call("pvst_class_vendor_size")(classIndex) >>> 0,
      (pointer, capacity) => call("pvst_class_vendor_write")(classIndex, pointer, capacity), `class ${uid} vendor`);
    const parameterCount = call("pvst_class_param_count")(classIndex) >>> 0;
    ...
    const kind = call("pvst_class_kind")(classIndex);
