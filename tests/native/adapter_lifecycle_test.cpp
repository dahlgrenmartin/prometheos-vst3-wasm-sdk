...
void every_failed_stage_unwinds_only_successful_initializations () {
  ...
  for (const auto& test : cases) {
    ...
    require (webvst_create (test.class_index, 48000., 128) == 0);
    require (fake_vst3::failures.live_objects == 0);
...
void connection_points_are_optional_but_partial_connections_are_unwound () {
  ...
  const auto without_component_connection = webvst_create (0, 48000., 128);
  ...
  webvst_destroy (without_component_connection);
  require (fake_vst3::failures.live_objects == 0);

  fake_vst3::failures.reset ();
  fake_vst3::failures.omit_controller_connection = true;
  const auto without_controller_connection = webvst_create (0, 48000., 128);
  require (without_controller_connection != 0);
  require (fake_vst3::failures.count (Call::ComponentConnect) == 0);
  require (fake_vst3::failures.count (Call::ControllerConnect) == 0);
  webvst_destroy (without_controller_connection);
  require (fake_vst3::failures.live_objects == 0);

  fake_vst3::failures.reset ();
  fake_vst3::failures.fail_at = fake_vst3::FailurePoint::ControllerConnect;
  require (webvst_create (0, 48000., 128) == 0);
...
void parameter_text_is_checked_utf8 () {
  ...
  require (webvst_class_param_title_size (0, 0) == expected_title.size ());
  require (webvst_class_param_title_write (0, 0, text.data (), static_cast<uint32_t> (expected_title.size () - 1)) == WEBVST_ERROR_BUFFER_TOO_SMALL);
  require (webvst_class_param_title_write (0, 0, text.data (), static_cast<uint32_t> (text.size ())) == WEBVST_OK);
  require (std::memcmp (text.data (), expected_title.data (), expected_title.size ()) == 0);
  text = {};
  require (webvst_class_param_value_text_size (0, 0, .75f) == expected_value.size ());
  require (webvst_class_param_value_text_write (0, 0, .75f, text.data (), static_cast<uint32_t> (text.size ())) == WEBVST_OK);
  require (std::memcmp (text.data (), expected_value.data (), expected_value.size ()) == 0);

  fake_vst3::failures.invalid_utf16 = true;
  require (webvst_class_param_title_size (0, 0) == 0);
  require (webvst_class_param_title_write (0, 0, text.data (), static_cast<uint32_t> (text.size ())) == WEBVST_ERROR_PLUGIN);
  require (webvst_class_param_value_text_size (0, 0, .75f) == 0);
  require (webvst_class_param_value_text_write (0, 0, .75f, text.data (), static_cast<uint32_t> (text.size ())) == WEBVST_ERROR_PLUGIN);
  fake_vst3::failures.reset ();
}
} // namespace

int lifecycle_test () {
  require (webvst_class_count () == 2);
  require (webvst_class_name_size (0) == 10);
  require (webvst_class_param_count (0) == 2);
  require (webvst_class_param_id (0, 0) == fake_vst3::kGainId);
  require (fake_vst3::factory.refs () == 1);
  ...
  const auto synth = webvst_create (0, 48000., 128);
  require (synth != 0);
  require (webvst_create (2, 48000., 128) == 0);
  require (webvst_create (0, NAN, 128) == 0);
  webvst_destroy (synth);
  require (webvst_reset (synth) == WEBVST_ERROR_HANDLE);
  require (fake_vst3::factory.refs () == 1);
  return 0;
}
