...
std::array<char, 33> class_uid (uint32_t class_index) {
  ...
  require (webvst_class_uid_size (class_index) == result.size () - 1);
  require (webvst_class_uid_write (class_index, result.data (), result.size () - 1) == WEBVST_OK);
  return result;
}
...
void instrument_generates_audio_after_a_vst3_note_event () {
  ...
  const auto instrument = webvst_create (0, 48000., 128);
  require (instrument != 0);
  require (webvst_note_on (instrument, 69, 1.f) == WEBVST_OK);
  require (webvst_process (instrument, nullptr, output.data (), 128) == WEBVST_OK);
  require (has_signal (output));
  webvst_destroy (instrument);
}
...
void effect_applies_its_gain_to_both_stereo_channels () {
  ...
  require (effect != 0);
  require (webvst_param_set (effect, 0x2001, .5f) == WEBVST_OK);
  require (webvst_process (effect, input.data (), output.data (), 128) == WEBVST_OK);
...
void effect_applies_gain_when_interleaved_input_and_output_alias () {
  ...
  require (effect != 0);
  require (webvst_param_set (effect, 0x2001, .5f) == WEBVST_OK);
  require (webvst_process (effect, interleaved.data (), interleaved.data (), 128) == WEBVST_OK);
...
void controllers_expose_stable_automatable_gain_parameters () {
  require (webvst_class_param_count (0) == 1);
  require (webvst_class_param_id (0, 0) == 0x1001);
  require ((webvst_class_param_flags (0, 0) & WEBVST_PARAMETER_AUTOMATABLE) != 0);
  require (webvst_class_param_step_count (0, 0) == 0);
  require (webvst_class_param_default (0, 0) == 1.f);
  require (webvst_class_param_count (1) == 1);
  require (webvst_class_param_id (1, 0) == 0x2001);
  require ((webvst_class_param_flags (1, 0) & WEBVST_PARAMETER_AUTOMATABLE) != 0);
  require (webvst_class_param_step_count (1, 0) == 0);
  require (webvst_class_param_default (1, 0) == .25f);
}
...
void instrument_parameter_state_restores_through_the_public_abi () {
  ...
  require (webvst_param_set (first, 0x1001, .625f) == WEBVST_OK);
  const auto state_size = webvst_state_size (first);
  require (state_size > 0);
  std::vector<uint8_t> state (state_size);
  require (webvst_state_write (first, state.data (), state_size) == WEBVST_OK);
  require (webvst_state_load (second, state.data (), state_size) == WEBVST_OK);
  require (webvst_param_get (second, 0x1001) == .625f);
