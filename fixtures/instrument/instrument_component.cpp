...
tresult PLUGIN_API InstrumentComponent::setBusArrangements (SpeakerArrangement*, int32 inputs, SpeakerArrangement*, int32 outputs) { return inputs == 0 && outputs == 1 ? kResultOk : kResultFalse; }
...
tresult PLUGIN_API InstrumentComponent::process (ProcessData& data) {
  if (data.symbolicSampleSize != kSample32 || data.numOutputs != 1 || !data.outputs || !data.outputs[0].channelBuffers32) return kInvalidArgument;
  ...
  if (data.inputEvents) for (int32 index = 0; index < data.inputEvents->getEventCount (); ++index) {
    Event event {}; if (data.inputEvents->getEvent (index, event) != kResultOk) continue;
    if (event.type == Event::kNoteOnEvent) { note_ = event.noteOn.pitch; velocity_ = event.noteOn.velocity; playing_ = true; }
    else if (event.type == Event::kNoteOffEvent) playing_ = false;
  }
  ...
  for (int32 frame = 0; frame < data.numSamples; ++frame) {
    ...
    phase_ += increment; if (phase_ >= 6.28318530717958647692) phase_ -= 6.28318530717958647692;
  }
  return kResultOk;
}
