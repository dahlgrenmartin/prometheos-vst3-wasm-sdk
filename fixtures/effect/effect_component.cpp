...
tresult PLUGIN_API EffectComponent::setBusArrangements (SpeakerArrangement*, int32 inputs, SpeakerArrangement*, int32 outputs) { return inputs == 1 && outputs == 1 ? kResultOk : kResultFalse; }
...
tresult PLUGIN_API EffectComponent::process (ProcessData& data) { if (data.symbolicSampleSize != kSample32 || data.numInputs != 1 || data.numOutputs != 1 || !data.inputs || !data.outputs || !data.inputs[0].channelBuffers32 || !data.outputs[0].channelBuffers32) return kInvalidArgument; apply_gain_changes (data.inputParameterChanges, gain_); for (int channel = 0; channel < 2; ++channel) for (int32 frame = 0; frame < data.numSamples; ++frame) data.outputs[0].channelBuffers32[channel][frame] = data.inputs[0].channelBuffers32[channel][frame] * static_cast<float> (gain_); return kResultOk; }
