...
Steinberg::tresult PLUGIN_API MemoryStream::queryInterface (const Steinberg::TUID, void** out) { if (out) *out = nullptr; return Steinberg::kNoInterface; }
Steinberg::tresult PLUGIN_API MemoryStream::read (void* buffer, Steinberg::int32 count, Steinberg::int32* read) {
  if (!buffer || count < 0) return Steinberg::kInvalidArgument; const auto available = bytes_.size () - position_; const auto take = std::min<size_t> (available, count); std::memcpy (buffer, bytes_.data () + position_, take); position_ += take; if (read) *read = static_cast<Steinberg::int32> (take); return take == static_cast<size_t> (count) ? Steinberg::kResultOk : Steinberg::kResultFalse;
}
...
Steinberg::tresult PLUGIN_API MemoryStream::seek (Steinberg::int64 offset, Steinberg::int32 mode, Steinberg::int64* result) {
  Steinberg::int64 base = mode == kIBSeekSet ? 0 : mode == kIBSeekCur ? static_cast<Steinberg::int64> (position_) : mode == kIBSeekEnd ? static_cast<Steinberg::int64> (bytes_.size ()) : -1; if (base < 0 || offset < -base) return Steinberg::kInvalidArgument; position_ = static_cast<size_t> (base + offset); if (result) *result = position_; return Steinberg::kResultOk;
}
