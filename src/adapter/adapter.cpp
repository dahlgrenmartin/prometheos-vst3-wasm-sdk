...
int32_t Adapter::class_vendor_write (uint32_t index, char* dst, uint32_t cap) const { TUID id {}; if (!class_id(index,id)) return PVST_ERROR_ARGUMENT; PFactoryInfo info {}; auto* f=GetPluginFactory (); const auto result=f && f->getFactoryInfo(&info)==kResultOk && write_text(info.vendor,dst,cap) ? PVST_OK : PVST_ERROR_BUFFER_TOO_SMALL; if(f)f->release(); return result; }
...
int32_t Adapter::reset(uint32_t handle){auto*v=instance(handle);return v?v->reset():PVST_ERROR_HANDLE;} int32_t Adapter::process(uint32_t h,const float*i,float*o,uint32_t n){auto*v=instance(h);return v?v->process(i,o,n):PVST_ERROR_HANDLE;}int32_t Adapter::note(uint32_t h,bool on,int32_t n,float velocity){auto*v=instance(h);return v?v->note(on,n,velocity):PVST_ERROR_HANDLE;}float Adapter::param_get(uint32_t h,uint32_t id){auto*v=instance(h);return v?v->get_parameter(id):0.f;}int32_t Adapter::param_set(uint32_t h,uint32_t id,float value){auto*v=instance(h);return v?v->set_parameter(id,value):PVST_ERROR_HANDLE;}
uint32_t Adapter::state_size(uint32_t h){auto*v=instance(h);if(!v)return 0;std::vector<uint8_t>b;return v->save_state(b)?static_cast<uint32_t>(b.size()):0;}int32_t Adapter::state_write(uint32_t h,uint8_t*dst,uint32_t cap){auto*v=instance(h);if(!v)return PVST_ERROR_HANDLE;std::vector<uint8_t>b;if(!v->save_state(b))return PVST_ERROR_PLUGIN;if(!dst||cap<b.size())return PVST_ERROR_BUFFER_TOO_SMALL;std::memcpy(dst,b.data(),b.size());return PVST_OK;}int32_t Adapter::state_load(uint32_t h,const uint8_t*src,uint32_t size){auto*v=instance(h);return v?v->load_state(src,size):PVST_ERROR_HANDLE;}
} // namespace pvst

extern "C" {
uint32_t pvst_abi_version(void){return PVST_ABI_VERSION;}uint32_t pvst_class_count(void){return pvst::adapter().class_count();}uint32_t pvst_class_uid_size(uint32_t i){return pvst::adapter().class_uid_size(i);}int32_t pvst_class_uid_write(uint32_t i,char*d,uint32_t c){return pvst::adapter().class_uid_write(i,d,c);}uint32_t pvst_class_name_size(uint32_t i){return pvst::adapter().class_name_size(i);}int32_t pvst_class_name_write(uint32_t i,char*d,uint32_t c){return pvst::adapter().class_name_write(i,d,c);}uint32_t pvst_class_vendor_size(uint32_t i){return pvst::adapter().class_vendor_size(i);}int32_t pvst_class_vendor_write(uint32_t i,char*d,uint32_t c){return pvst::adapter().class_vendor_write(i,d,c);}uint32_t pvst_class_kind(uint32_t i){return pvst::adapter().class_kind(i);}uint32_t pvst_class_param_count(uint32_t i){return pvst::adapter().class_param_count(i);}uint32_t pvst_class_param_id(uint32_t i,uint32_t p){return pvst::adapter().class_param_id(i,p);}uint32_t pvst_class_param_flags(uint32_t i,uint32_t p){return pvst::adapter().class_param_flags(i,p);}uint32_t pvst_class_param_step_count(uint32_t i,uint32_t p){return pvst::adapter().class_param_step_count(i,p);}float pvst_class_param_default(uint32_t i,uint32_t p){return pvst::adapter().class_param_default(i,p);}uint32_t pvst_class_param_title_size(uint32_t i,uint32_t p){return pvst::adapter().class_param_title_size(i,p);}int32_t pvst_class_param_title_write(uint32_t i,uint32_t p,char*d,uint32_t c){return pvst::adapter().class_param_title_write(i,p,d,c);}uint32_t pvst_class_param_value_text_size(uint32_t i,uint32_t p,float v){return pvst::adapter().class_param_value_text_size(i,p,v);}int32_t pvst_class_param_value_text_write(uint32_t i,uint32_t p,float v,char*d,uint32_t c){return pvst::adapter().class_param_value_text_write(i,p,v,d,c);}uint32_t pvst_create(uint32_t i,double r,uint32_t f){return pvst::adapter().create(i,r,f);}void pvst_destroy(uint32_t h){pvst::adapter().destroy(h);}int32_t pvst_reset(uint32_t h){return pvst::adapter().reset(h);}int32_t pvst_process(uint32_t h,const float*i,float*o,uint32_t n){return pvst::adapter().process(h,i,o,n);}int32_t pvst_note_on(uint32_t h,int32_t n,float v){return pvst::adapter().note(h,true,n,v);}int32_t pvst_note_off(uint32_t h,int32_t n){return pvst::adapter().note(h,false,n,0.f);}float pvst_param_get(uint32_t h,uint32_t i){return pvst::adapter().param_get(h,i);}int32_t pvst_param_set(uint32_t h,uint32_t i,float v){return pvst::adapter().param_set(h,i,v);}uint32_t pvst_state_size(uint32_t h){return pvst::adapter().state_size(h);}int32_t pvst_state_write(uint32_t h,uint8_t*d,uint32_t c){return pvst::adapter().state_write(h,d,c);}int32_t pvst_state_load(uint32_t h,const uint8_t*s,uint32_t z){return pvst::adapter().state_load(h,s,z);}
}
